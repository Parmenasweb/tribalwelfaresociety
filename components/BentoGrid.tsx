    ...
    >
      {cards.map((card, i) => (
        <motion.div 
          ...
          className={`${card.size} bg-white/40 border-[1.5px] border-stone-200/60 rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-12 flex flex-col justify-between group cursor-pointer shadow-[0_10px_40px_rgba(0,0,0,0.02)] hover:shadow-[0_40px_80px_rgba(0,0,0,0.06)] transition-all duration-700 relative overflow-hidden hover:scale-[1.01]`}
        >
          ...
          <div className="relative z-10">
            <h3 className="serif text-3xl md:text-4xl mb-4 text-stone-900 leading-tight group-hover:translate-x-2 transition-transform duration-500">{card.title}</h3>
            <p className="text-stone-500 text-sm md:text-base font-light leading-relaxed group-hover:text-stone-700 transition-colors duration-500">
              {card.desc}
            </p>
          </div>
        </motion.div>
